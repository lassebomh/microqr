 ...
 */
AlignmentPattern.getRowColCoords = function getRowColCoords(version) {
  if (version === 1) return [];
 ...
 */
AlignmentPattern.getPositions = function getPositions(version) {
  const coords = [];
  const pos = AlignmentPattern.getRowColCoords(version);
 ...
 */
Mode.getCharCountIndicator = function getCharCountIndicator(mode, version) {
 ...
 */
Mode.getBestModeForData = function getBestModeForData(dataStr) {
  if (Regex.testNumeric(dataStr)) return Mode.NUMERIC;
  else if (Regex.testAlphanumeric(dataStr)) return Mode.ALPHANUMERIC;
  else if (Regex.testKanji(dataStr)) return Mode.KANJI;
 ...
 */
Mode.toString = function toString(mode) {
 ...
 */
Mode.isValid = function isValid(mode) {
 ...
 */
Mode.fromString = function fromString(string) {
  if (typeof string !== "string") {
    throw new Error("Param is not a string");
  }
 ...
 */
Mode.from = function from(value, defaultValue) {
  if (Mode.isValid(value)) {
    return value;
  }
...
function getBestVersionForDataLength(mode, length, errorCorrectionLevel) {
  for (let currentVersion = 1; currentVersion <= 40; currentVersion++) {
    if (
      length <= Version.getCapacity(currentVersion, errorCorrectionLevel, mode)
    ) {
...
function getBestVersionForMixedData(segments, errorCorrectionLevel) {
  for (let currentVersion = 1; currentVersion <= 40; currentVersion++) {
    ...
    if (
      length <=
      Version.getCapacity(currentVersion, errorCorrectionLevel, Mode.MIXED)
    ) {
      return currentVersion;
    }
  }
 ...
 */
Version.from = function from(value, defaultValue) {
 ...
 */
Version.getCapacity = function getCapacity(
  version,
  errorCorrectionLevel,
  mode
) {
 ...
 */
Version.getBestVersionForData = function getBestVersionForData(
  data,
  errorCorrectionLevel
) {
  let seg;
 ...
 */
Version.getEncodedBits = function getEncodedBits(version) {
 ...
 */
Segments.fromArray = function fromArray(array) {
 ...
 * @return {Array}          Array of segments
 */
Segments.fromString = function fromString(data, version) {
  ...
  return Segments.fromArray(mergeSegments(optimizedSegs));
};
 ...
 * @return {Array}       Array of segments
 */
Segments.rawSplit = function rawSplit(data) {
  return Segments.fromArray(
